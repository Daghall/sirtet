...
export default class Renderer {
  ...
  constructor(canvasElement, board, scoreKeeper) {
    ...
    this.#addAnimation("start screen", () => {
      return this.#drawStartScreen();
    });
    ...
    this.#board.on("pause game", () => {
      ...
      this.#addAnimation("paused", () => {
        // Overlay
        this.#canvas.fillStyle = colors.overlay;
        this.#canvas.beginPath();
        this.#canvas.rect(0, 0, width, height);
        this.#canvas.fill();

        // Text
        this.#canvas.fillStyle = colors.startScreen;
    ...
    this.#board.on("start game", () => {
      this.#removeAnimation("start screen");
      this.#renderCountdown(() => {
        this.#board.setState(Board.STATES.PLAYING);
      });
    });
  ...
  #drawStartScreen() {
    ...
    const logoStart = (width - 21 * blockWithPadding) / 2;
    ...
    this.#canvas.fillStyle = colors.startScreen;
    this.#canvas.beginPath();
    Renderer.LOGO_BLOCKS.forEach(({ x, y }) => {
      this.#canvas.rect(logoStart + x * blockWithPadding, logoStart + y * blockWithPadding, blockSize, blockSize);
    });
    this.#canvas.fill();

    // Text
    this.#canvas.textAlign = "center";
    this.#canvas.font = fonts.startScreen;
    this.#canvas.fillText("ENTER to start game", width / 2, height / 2);
    return true;
  }
  ...
  #renderCountdown(callback) {
    ...
    const signals = [
      { time: 3000, action: () => this.#board.emit("countdown") },
      { time: 2000, action: () => this.#board.emit("countdown") },
      { time: 1000, action: () => this.#board.emit("countdown") },
      { time: 0, action: () => this.#board.emit("countdown finished") },
    ];
    ...
    this.#addAnimation("starting", (time) => {
      ...
      if (countdown < signals[0].time) {
        signals.shift().action();
      }
  ...
  #renderLevel() {
    ...
    const timeString = this.#formatTime(this.#board.getPlayTime());
